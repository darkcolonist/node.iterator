var settings = {
  ...
  /**
   * number of latest iterations to show (int)
   */
  logs_to_show: 20,
  ...
  /**
   * expiration time of logs in logs.db. (days)
  ...
  /**
   * if this number is reached, iterator will automatically pause to
   * conserve resources
   * @type {Number}
   */
...
db_iterators = new nedb({ filename: "data/iterators.db", autoload: true });
db_logs = new nedb({ filename: "data/logs.db", autoload: true });
...
util = {
  ...
  prune_databases : function(){
    var max_date = moment()
      .subtract(settings.max_age_of_logs, "days")
      .format('YYYY-MM-DD HH:mm:ss');

    async.waterfall([
      function prune_errors(waterfallCB){
        db_errors.remove({ timestamp : { $lte : max_date } }, { multi: true }, 
          waterfallCB);
      },
      function prune_logs(waterfallCB){
        db_logs.remove({ timestamp : { $lte : max_date } }, { multi: true }, 
          waterfallCB);
      },
      function recall_prune(waterfallCB){
        setTimeout(util.prune_databases, settings.db_compact_frequency);
      }
    ], function(error){
      if(error){
        console.log("database pruning error occurred!");
      }
    });
  },

  delete_aged_logs : function(){
    var max_date = moment()
      .subtract(settings.max_age_of_errors, "hours")
      .format('YYYY-MM-DD HH:mm:ss');

    db_errors.remove({ timestamp : { $lte : max_date } }, { multi: true }, function (err, numRemoved) {
      setTimeout(util.delete_aged_logs, settings.db_compact_frequency);
    });
  }
}
...
db_iterators.persistence.setAutocompactionInterval(settings.db_compact_frequency);
db_logs.persistence.setAutocompactionInterval(settings.db_compact_frequency);
db_errors.persistence.setAutocompactionInterval(settings.db_compact_frequency);

/**
 * initialize errors.db pruning
 */
util.prune_databases();
